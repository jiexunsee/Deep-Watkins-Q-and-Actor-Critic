...
class PolicyGradientAgent(object):
    ...
    def train_step(self, obs, acts, advantages):
        ...
        loss, _ = self._s.run([self.loss, self._train], feed_dict=batch_feed)
        return loss
...
def process_rewards(rews): # CHANGE HERE TO USE VALUE FUNCTION BASELINE?
...
def main():
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for batch in range(eparams['num_batches']):
            ...
            loss = agent.train_step(b_obs, b_acts, b_rews)
            print(loss)
