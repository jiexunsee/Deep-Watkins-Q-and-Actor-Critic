...
class PolicyLearner:
	def __init__(self, n_actions, n_states, discount=0.8, alpha=0.1, beta=0.1, lambda_w=0.5, lambda_theta=0.5):
		...
		self.state_tensor, self.value_tensor, self.chosen_action_index, self.already_chosen, self.log_chosen_action_tensor, self.opt, self.theta, self.w = self._build_model()

		self.w_grads_and_vars = self._get_w_grads_and_vars(self.opt)
		self.theta_grads_and_vars = self._get_theta_grads_and_vars(self.opt)
		...
		self.apply_w_placeholder_op = self.opt.apply_gradients(self.w_grad_placeholder)
		self.apply_theta_placeholder_op = self.opt.apply_gradients(self.theta_grad_placeholder)
	...
	def _build_model(self):
		...
		w = tf.Variable(tf.zeros(shape=(self.n_states, 1)), name='value_weight')
		...
		# chosen_action_index = tf.multinomial(action_logits, num_samples=1)
		...
		# NOTE: IFFY TO USE LOGITS AND NOT PROBABILITY IN LOG AND LOG GRADIENT CALCULATION
		# chosen_action_prob_tensor = tf.gather(action_logits, already_chosen, axis=1)
		log_chosen_action_tensor = tf.log(chosen_action_prob_tensor)
		opt = tf.train.GradientDescentOptimizer(self.alpha)

		return state_tensor, value_tensor, chosen_action_index, already_chosen, log_chosen_action_tensor, opt, theta, w

	def _get_w_grads_and_vars(self, opt):
		w_variables = [var for var in tf.global_variables() if 'value' in var.op.name] # tf.get_variable() only works if the variable was created using tf.get_variable()
		w_grads_and_vars = opt.compute_gradients(self.value_tensor, w_variables)
		return w_grads_and_vars

	def _get_theta_grads_and_vars(self, opt):
		theta_variables = [var for var in tf.global_variables() if 'policy' in var.op.name]
		theta_grads_and_vars = opt.compute_gradients(self.log_chosen_action_tensor, theta_variables)
		return theta_grads_and_vars
	...
	def learn(self, state, action, next_state, reward): # ignoring the I*lambda update thing for now
